// rvtest_config.h
//
// C-level ACT4 test configuration for riscv-cpu-rv32i (this repo's 5-stage
// pipelined `top` module).
//
// This mirrors riscv-cpu-rv32i.yaml (RV32I only, Sm "implemented" only
// because UDB gates MXLEN/other base hart params on it, every optional
// M-mode feature actually turned off - no CSR file, no PMP, no privilege
// modes below M, no traps other than the core's own illegal-instruction
// halt) and test_config.yaml (include_priv_tests: False).
//
 ...
 * Zicsr, Zifencei, and all Zk, Zv, Zs, Sv, Ss extensions - is absent.
 */
...
/***** Privilege modes *****
 * Sm is nominally "implemented" per riscv-cpu-rv32i.yaml only because UDB
 * requires a base hart to gate MXLEN/other params on it; this core has no
 * CSR file (misa/marchid/mimpid/mtvec/etc. simply don't exist) and no
 * M-mode features below are actually present. No S-mode, no U-mode.
 */
...
/***** PMP / HPM counters *****
 * No PMP entries, no hardware performance monitor counters beyond what's
 * mandatory (none).
 */

#define RVTEST_NUM_PMP_ENTRIES 0
...
#define RVTEST_MISALIGNED_LDST_ALLOWED 1

/***** Physical memory *****/

#define RVTEST_PHYS_ADDR_WIDTH 32
